...
from dogqc.indexJoins import IndexJoinTranslator, IndexScanTranslator
...
class CudaCompiler ( object ):
    ...
    def __init__ ( self, dbpath, algebraContext, smArchitecture = "sm_52", decimalRepr = CType.FP32, debug = False ):
    ...
    def buildContext(self, translationPlan):
        self.codegen = CodeGenerator ( self.decimalRepr )
        self.vars = Vars ()
        self.attFile = PipelineAttributesFile ( self.algebraContext, self.vars, self.codegen, self.dbpath )
    ...
    def compile ( self, filename, compileOption ):
        return self.codegen.compile_ ( filename, compileOption, self.smArchitecture, self.debug  )
  
    def execute ( self, deviceid=None, timeout=None ):
        return self.codegen.execute (deviceid, timeout) 
    ...
    def translate ( self, algebraExpr, child1=None, child2=None ): 
        ...
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.Selection ):
            if KernelCall.args.dogqc_lane_refill and algebraExpr.doLB:
                op = LaneRefill( algebraExpr, SelectionTranslator ( algebraExpr, child1 ) )
            else:
        ...
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.MultiMap ):
            op = MultiMapTranslator ( algebraExpr, child1 )
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.EquiJoin ):
            if KernelCall.args.dogqc_lane_refill and algebraExpr.doLB:
                op = LaneRefill( algebraExpr,  EquiJoinTranslator ( algebraExpr, child1, child2 ))
            else:
        ...
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.Aggregation ):
            if KernelCall.args.dogqc_lane_refill and algebraExpr.doLB:
                op = LaneRefill( algebraExpr, AggregationTranslator ( algebraExpr, child1 ))
            else:
        ...
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.IndexJoin ):
            if KernelCall.args.dogqc_lane_refill and algebraExpr.unique:
                op = IndexJoinTranslator ( algebraExpr, child1 ) # LaneRefill( algebraExpr, IndexJoinTranslator ( algebraExpr, child1 ))
            else:
                op = IndexJoinTranslator ( algebraExpr, child1 )
                #op = LaneRefill( algebraExpr, IndexJoinTranslator ( algebraExpr, child1 ))
        if isinstance ( algebraExpr, dogqc.relationalAlgebra.IndexScan ):
            op = IndexScanTranslator ( algebraExpr, child1 )
...
class PipelineAttributesFile ( object ):
    ...
    def __init__ ( self, algebraContext, vars, codegen, dbpath ):
        ...
        self.indexColumns = dict()
    
    def file ( self, var ):
        return '{}/{}'.format(self.dbpath, var.get())


    def mapIndex (self, rel_name):

        dbCol = self.indexColumns (  )

        indexColFileSys = '{}/{}'.format(self.dbpath, rel_name)
        if indexColFileSys not in self.indexColumns:
            a.declarePointer ( self.codegen.read )
            emit ( assign ( a, mmapFile ( a.dataType, indexColFileSys )), self.codegen.read )
            self.codegen.gpumem.mapForRead ( a )
            self.indexColumns [ indexColFileSys ] = a.getGPU()
        else:
            a.declarePointer ( self.codegen.read )
            emit ( assign ( a, mmapFile (a.dataType, indexColFileSys)), self.codegen.read)
            self.codegen.gpumem.declare (a)
            emit ( assign ( a.getGPU(), self.indexColumns[ indexColFileSys ] ), self.codegen.gpumem.cudaMalloc )
    ...
    def dematerializeAttribute ( self, a, tid, scanTableId=None ):
    ...
    def indexColumns ( self, a, identifier, size ):
        var1 = Variable ( CType.INT, identifier + "_offset",  size )
        var2 = Variable ( CType.INT, identifier + "_position", size )
        return [var1, var2]
...
class ScanLoop ( object ):
    ...
    def __init__ ( self, tid, scanType, isTempScan, table, scanRelation, algExpr, ctxt ):
        ...
        if scanType == scanType.KERNEL:
            ...
            vars.divergenceFlushVar = vars.flushVar
            ...
            emit ( assign ( vars.flushVar, notLogic ( ballotIntr( qlib.Const.ALL_LANES, smaller( add( vars.loopVar, intConst(32)), table['size'] ) ) ) ), codegen )                
        ...
        for (id, a) in scanRelation.items():
            print("scanLoop", algExpr.opId, "attr", id, a.name)
            if id == algExpr.tid.id:
                v = Variable ( CType.INT, "att{}_tid".format(a.id), 0)
                #v.declare(ctxt.codegen)
                ctxt.attFile.locFile[id] = AttributeLocation.TABLE
                #ctxt.codegen.gpumem.declare ( v )
                #ctxt.codegen.currentKernel.addVar(v)
            else:
                if not isTempScan:
        ...
        with IfClause ( ctxt.vars.activeVar, ctxt.codegen ):
            for id, a in scanRelation.items():
                if id == algExpr.tid.id:
                    ...
                    emit ( assign ( regVar, "tid_{}{}".format(algExpr.tableName, algExpr.scanTableId) ), ctxt.codegen )
                else:
                    ctxt.attFile.dematerializeAttribute ( a, tid )
                #print(id, a.name)
#                if not isTempScan:
#                    ctxt.attFile.dematerializeAttribute ( a, tid, algExpr.scanTableId, algExpr )
#                else:
#                    ctxt.attFile.dematerializeAttribute ( a, tid )
    def __enter__ ( self ): 
        return self
...
class ScanTranslator ( LiteralTranslator ): 
    ...
    def produce ( self, ctxt ):
        ...
        print(self.algExpr.outRelation)
...
class MultiMapTranslator ( UnaryTranslator ):
    ...
    def consume ( self, ctxt ):
        commentOperator ("multimap", self.algExpr.opId, ctxt.codegen)

        # for map strings we set a reference to the char heap for the new attribute
        
        for attr in self.algExpr.mapAttrs:
            ctxt.attFile.declareRegister ( attr )

        profile_lane_activity(ctxt)

        with IfClause ( ctxt.vars.activeVar, ctxt.codegen ):
            for i, attr in enumerate(self.algExpr.mapAttrs):
                emit ( assign ( ctxt.attFile.access(attr), self.algExpr.attrs[i][1].translate(ctxt)), ctxt.codegen)
        self.parent.consume( ctxt )
