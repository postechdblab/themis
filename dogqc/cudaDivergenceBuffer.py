...
class BufferHelperSharedMemory ( object ):
    ...
    def consumeFlushToBuffer ( self, numactive, mask ):
        ...
        with IfClause ( larger ( numactive, intConst(0) ), ctxt.codegen ):
            ...
            emit ( assign ( self.scan, add ( popcount ( andBitwise ( mask, codegen.prefixlanes() ) ), self.buffercount ) ), ctxt.codegen )
            ...
            emit ( assignAdd ( self.buffercount, numactive ), ctxt.codegen )
            emit ( assign ( ctxt.vars.activeVar, intConst(0) ), ctxt.codegen )
    ...
    def bufWriteReg ( self, numactive, mask ):
        ...
        emit ( assign ( self.activeDest, andLogic ( largerEqual ( codegen.warplane(), self.buffercount ), smaller ( codegen.warplane(), add ( self.buffercount, numactive ) ) ) ), ctxt.codegen )
    ...
    def consumeRefillFromBuffer ( self, numactive, mask, threshold ):
        ...
        with IfClause ( andLogic ( smaller ( numactive, intConst(threshold) ), self.buffercount), codegen ):
            emit ( assign ( self.numRemaining, maxMath ( sub ( add ( self.buffercount, numactive ), intConst(32) ), intConst(0) ) ), codegen )
            ...
            emit ( assign ( self.buffercount, self.numRemaining ), codegen )

    def bufReadSmem ( self, numactive, mask ):
        ctxt = self.ctxt
        codegen = self.ctxt.codegen
        with IfClause ( andLogic ( notLogic ( ctxt.vars.activeVar ), smaller ( self.scan, self.buffercount ) ), codegen ):
    ...
    def bufReadReg ( self, numactive, mask ):
        ...
        emit ( assign ( self.activeDest, andLogic ( notLogic ( ctxt.vars.activeVar ), smaller ( self.scan, self.buffercount ) ) ), codegen )
        emit ( assign ( self.activeSource, andLogic ( largerEqual ( codegen.warplane(), self.numRemaining ), smaller ( codegen.warplane(), self.buffercount ) ) ), codegen ) 
...
class BufferedLoop ( object ):
    ...
    def __init__ ( self, ctxt, bufferVars, opId, threshold=0.8 ):
        ...
        self.bufferHelper.buffercount = buffercount
    ...
    def open ( self ):
        ...
        emit ( assign ( declare ( self.bailout ), inlineIf( ctxt.vars.divergenceFlushVar, intConst(0), intConst(self.threshold))), ctxt.codegen )    

        self.outerBuffers = dict()
        for v in self.bufferVars:
            buf = Variable.val ( v.dataType, 'outer_' + ident.divergenceBuffer ( v ) + "_" + str(self.opId) + "_" )
            buf.declareAssign ( v, ctxt.codegen ) 
            self.outerBuffers[v.get()] = buf
    


        # starts main buffer loop
        self.whileLoop = WhileLoop ( larger ( add ( self.buffercount, self.numactive ), self.bailout ), ctxt.codegen )
        ...
        #emit ( "active_lanes_num += __popc(__ballot_sync(ALL_LANES, active))", ctxt.codegen )
        #emit ( "oracle_active_lanes_num += 32", ctxt.codegen )
    ...
    def close ( self ):
        ...
        for v in self.bufferVars:
            buf = self.outerBuffers[v.get()]
            emit ( assign ( v, buf ), ctxt.codegen )
