...
__forceinline__ __device__ bool indexProbeMulti(int* val, int* offset, int& matchOffset, int& matchEnd, int v) {
    int s = matchOffset;
    int e = matchEnd;
    //assert(s < e);
    while (s < e) {
        ...
        if (nv < v) {
            ...
            matchOffset = offset[2*tid];
            matchEnd = offset[2*tid+1];
            return true;
        }
    }
    matchOffset = matchEnd = 0;
    return false;
}

__forceinline__ __device__ bool relProbeMulti ( int* offsets, int tid, int& offset, int& end ) {
    offset = offsets[2*tid];
    end = offsets[2*tid+1];
    return offset < end;
}

__forceinline__ __device__ int indexGetTid ( int* t, int lid ) {
    return t[lid];
}
